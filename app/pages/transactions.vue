...
const onFileSelected = async (event: Event) => {
  ...
  try {
    ...
    if (result) {
      ...
      txName.value = result.suggestedTransactionName || result.storeName || "Receipt Scan";
      txAmount.value = result.totalAmount || 0;
      txType.value = "spending";
      txNotes.value = `Scanned from receipt at ${result.storeName || "Unknown Merchant"}.\n\nItems:\n` +
        result.items.map(item => `- ${item.name}: Rp. ${item.price.toLocaleString("id-ID")}`).join("\n");

      const matchedCat = categories.value.find(
        (c) => c.name.toLowerCase() === result.suggestedCategory.toLowerCase() && c.type === "spending"
      );
      if (matchedCat) {
...
const filteredTransactions = computed(() => {
  return transactions.value.filter((t) => {
    ...
    const matchesCategory = selectedCategoryId.value === "all" || t.categoryId === selectedCategoryId.value;
  ...
  <div>
    ...
    <UDashboardGroup>
      ...
      <UDashboardPanel>
        ...
        <!-- Filters Row -->
        <div class="flex flex-col sm:flex-row gap-4 items-center justify-between bg-neutral-50 dark:bg-neutral-900/40 p-4 rounded-2xl border border-neutral-200/50 dark:border-neutral-800/50">
            ...
            <div class="flex items-center gap-4 flex-1 min-w-0">
              ...
              <div
                :class="[
                  ...
                  colorClassMap[tx.categoryColor || 'slate'] || colorClassMap.slate
                ]"
              >
                <UIcon :name="tx.categoryIcon || 'i-lucide-folder'" class="w-5 h-5" />
              </div>

              <!-- Transaction Info -->
              <div class="flex-1 min-w-0">
                <p class="font-bold text-neutral-850 dark:text-neutral-100 truncate">
                  {{ tx.name }}
                </p>
                ...
                <p v-if="tx.notes" class="text-xs text-neutral-400 dark:text-neutral-500 mt-1 truncate">
                  {{ tx.notes }}
                </p>
              </div>
            </div>
        ...
        >
          <UIcon name="i-lucide-arrow-right-left" class="w-12 h-12 text-neutral-400 dark:text-neutral-600 mb-3" />
          <p class="font-semibold text-neutral-700 dark:text-neutral-300">No transactions found</p>
          <p class="text-sm text-neutral-500 dark:text-neutral-500 mt-1 mb-4">Try adjusting your filters or add a new transaction</p>
          <UButton label="Add Transaction" icon="i-lucide-plus" size="sm" @click="openAddModal" />
        </div>
      </UContainer>
        ...
        <UContainer class="p-6 flex flex-col gap-5 w-full">
          ...
          <div>
            ...
            <USelect
              ...
              :items="formCategories.map((c) => ({ label: c.name, value: c.id }))"
              class="w-full"
    ...
    <UModal v-model:open="isScanning" prevent-close>
      <template #content>
        <div class="flex flex-col items-center justify-center p-8 gap-4">
          ...
          <p class="font-semibold text-neutral-850 dark:text-neutral-100">Scanning receipt image...</p>
          <p class="text-sm text-neutral-500">Parsing items and finding best category matches with Gemini AI</p>
        </div>
      </template>
    </UModal>
      </UDashboardPanel>
